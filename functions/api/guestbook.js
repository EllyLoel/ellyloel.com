...
export async function onRequestPost(context) {
	try {
		...
		postData.comments.push({
			...
			html: output.message
				? markdownit({
						...
						linkify: true,
						typographer: true,
					}).render(output.message).trim()
				: undefined,
			scribble: output.scribble || undefined,
			scribbleAlt: output["scribble-alt"] || undefined,
		});
		...
		await octokit.rest.repos.createOrUpdateFileContents({
			...
			owner: REPO_OWNER,
			path: `src/input/content/pages/${postName}.11tydata.json`,
			repo: REPO_NAME,
			sha,
		});
